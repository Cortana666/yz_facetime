...
class Teacher {
    ...
    public static function invite($connection, $ws_worker, $data) {
        if ($ws_worker->room[$connection->room_id]['member'][$data['user_id']]['connection']) {
            $ws_worker->room[$connection->room_id]['member'][$data['user_id']]['connection']->send(Base::success('invite'));
        }
    }
    ...
    public static function hangUp($connection, &$ws_worker, $data) {
        foreach ($ws_worker->room[$connection->room_id]['member'] as $key => $value) {
            if ($value['type'] == 3 && $value['step'] == 3 && $value['connection']) {
                $ws_worker->room[$connection->room_id]['member'][$key]['step'] = 4;
